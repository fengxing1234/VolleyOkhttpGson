...
public abstract class BeanTools {
	 ...
	 * @param entityClass
	 *            实体的类型
	 * @return data 返回包含两个数组的HashMap，可参考以下使用方法： String[] fieldName = (String[])
	 ...
	 * @return
	 */
	public static Map<String, Object> getAllFiledValue(Object object) {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			fieldType[i] = fields[i].getType();// 组装类型数组
			try {
				Object fieldValue = getPrivateProperty(object, fieldName[i]);
				if (fieldValue == null) {
					// if (fieldType[i].equals(String.class)) {
					// data.put(fieldName[i], "");
					// } else if (fieldType[i].equals(Integer.class)) {
					// data.put(fieldName[i], 0);
					// }
					continue;
				} else {
					data.put(fieldName[i], fieldValue);
				}
			} catch (IllegalAccessException e) {
				e.printStackTrace();
			} catch (NoSuchFieldException e) {
